...
summary: 'TestLocalConnectivity メソッドを使用して、アクティブなクラスター内の各ノードの Cluster IP \(CIP\) を ping できます。' 
---
= ローカル接続のテスト
:allow-uri-read: 
:icons: font
...
[role="lead"]
使用することができます `TestLocalConnectivity`アクティブ クラスター内の各ノードのクラスター IP (CIP) を ping する方法。
...
詳細
 a| 
ローカルのアクティブ クラスター内の各ノードの個別の ping 応答時間。
 a| 
JSONオブジェクト
