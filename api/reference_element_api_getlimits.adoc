...
summary: GetLimits メソッドを使用して、API によって設定された制限値を取得できます。これらの値は Element のリリース間で変更される可能性がありますが、システムを更新しない限り変更されません。  API によって設定された制限値を知っておくと、ユーザー向けツールの API スクリプトを作成するときに役立ちます。 
---
= 制限を取得
:allow-uri-read: 
:icons: font
...
[role="lead"]
使用することができます `GetLimits`API によって設定された制限値を取得するメソッド。これらの値は Element のリリース間で変更される可能性がありますが、システムを更新しない限り変更されません。  API によって設定された制限値を知っておくと、ユーザー向けツールの API スクリプトを作成するときに役立ちます。


NOTE: その `GetLimits`このメソッドは、メソッドを渡すために使用された API エンドポイントのバージョンに関係なく、現在のソフトウェア バージョンの制限を返します。
...
このメソッドは、API 制限を含む名前と値のペアを持つ JSON オブジェクトを返します。
