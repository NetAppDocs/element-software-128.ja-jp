...
summary: ListVirtualVolumeTasks メソッドを使用して、システム内の仮想ボリューム タスクのリストを取得できます。 
---
= 仮想ボリュームタスクの一覧
:allow-uri-read: 
:icons: font
...
[role="lead"]
使用することができます `ListVirtualVolumeTasks`システム内の仮想ボリューム タスクのリストを取得するメソッド。
...
仮想ボリュームタスクID
 a| 
情報を取得する仮想ボリューム タスク ID のリスト。このパラメータを省略すると、メソッドはすべての仮想ボリューム タスクに関する情報を返します。
 a| 
UUID配列
 a| 
なし
...
tasks
 a| 
クラスター内の仮想ボリューム タスクを記述するオブジェクトのリスト。
 a| 
