...
summary: GetSSLCertificate メソッドを使用して、クラスターのストレージ ノードで現在アクティブな SSL 証明書を取得できます。 
---
= SSL証明書を取得する
:allow-uri-read: 
:icons: font
...
[role="lead"]
使用することができます `GetSSLCertificate`クラスターのストレージ ノード上で現在アクティブな SSL 証明書を取得する方法。
